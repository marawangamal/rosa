...
def factorize(args, model, lr_scheduler, optimizer, epoch, steps_counter, num_training_steps):
    # Mask gradients
    with torch.no_grad():
        logging.info("\n=> [Epoch {}] Factorizing model at step {} with factorize level {} **\n".format(
            epoch, steps_counter, args['pnmodel']['factorize_level']
            )
        )
...
def train_epoch(args, model, device, train_dataloader, optimizer, lr_scheduler, epoch, print_freq=10,
    ...
    for i_step, batch in enumerate(train_dataloader):
        ...
        # Masking PEFTNet gradients
        latency_report.start()
        if args['pnmodel']['name'] == 'rosa' and args['pnmodel']['factorize_level'] == "batch":
            num_training_steps = args['train']['epochs'] * len(train_dataloader)

            model, optimizer, lr_scheduler = factorize(
                args, model, lr_scheduler, optimizer, epoch, steps_counter, num_training_steps
            )
...
def train(
    ...
    for i_epoch in range(curr_epoch, args["train"]["epochs"] + 1):
        ...
        if args['pnmodel']['name'] == "rosa" and args['pnmodel']['factorize_level'] == "epoch" and \
                (i_epoch == 0 or i_epoch % args['pnmodel']['factorize_freq'] == 0):
            num_training_steps = args['train']['epochs'] * len(train_dataloader)
            cmodel, optimizer, lr_scheduler = factorize(
                args, cmodel, lr_scheduler, optimizer, i_epoch, steps_counter, num_training_steps
            )
        ...
        if i_epoch > 0:
            ...
            train_metrics, optimizer, steps_counter = train_epoch(
                ...
                print_freq=args["print_freq"], writer=writer, steps_counter=steps_counter
            )
        else:
            train_metrics = dict()
...
def main(cfg: DictConfig):
    ...
    for t in range(max(1, args["runs"])):
        ...
        exp_name = get_experiment_name(
            {"train": args["train"], "model": args["model"], "pnmodel": args["pnmodel"], "trial": t}
        )
        ...
        logging.info("=> Using {} model ...".format(args['pnmodel']['name'].lower()))
        ignore_list = get_ignore_list_glue(model) if args['pnmodel']['ignore_list'] else None
        # cmodel = {
        #     "rosa": pn.RosaNet,
        #     "lora": pn.LoraNet,
        #     "ia3": pn.IA3Net,
        #     "none": lambda x, **kwargs: x
        # }[args['pnmodel']['name'].lower()](model, ignore_list=ignore_list, **args['pnmodel']['params'])
        if args['pnmodel']['name'].lower() != "none":
            logging.info("=> Using {} PEFT Method ...".format(args['pnmodel']['name'].lower()))
            cmodel = pn.PEFTNet(
                model,
                method=args['pnmodel']['name'].lower(),  # PEFT method ['rosa', 'lora', 'ia3']
        ...
        cuda_memory_tracker.track('[main] Created PEFT model loaded to cpu')
        ...
        if dct_latest is not None:
            ...
            torch.cuda.empty_cache()
            cuda_memory_tracker.track('[main] Moved PEFT model loaded to gpu')
